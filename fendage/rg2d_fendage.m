...
theta1      = 0;%pi/2;     %3.7296;%pi;%pi/2; 3.7083;%
theta2      = pi;%3*pi/2;      %0.58800;%0%3*pi/2; 5.5975;% % Initial angles of the crack
...
[ nodes,elements,ntoelem,boundary,order] = readmesh( 'meshes/fendage/plate_c_squared.msh' );
nnodes = size(nodes,1);
...
elim1 = find(xy1234(1,:)>10); elim2 = find(xy1234(2,:)>10);
...
[ nodes2,elements2,ntoelem2,boundary2,order] = readmesh( 'meshes/fendage/plate_nu.msh' );
...
ur1 = UR(:,1); fr1 = UR(:,5)*40; % 40 is the ratio between mesh sizes (hack)
ur2 = UR(:,2); fr2 = UR(:,6)*40;
ur3 = UR(:,3); fr3 = UR(:,7)*40;
ur4 = UR(:,4); fr4 = UR(:,8)*40;
...
[ nodesu,elementsu,ntoelemu,boundaryu,orderu] = readmesh( 'meshes/fendage/plate_nu.msh' );
...
nodes3s = nodes3r + 1e-2*ones(size(nodes3r,1),1)*n';
nodes3i = nodes3r - 1e-2*ones(size(nodes3r,1),1)*n';
