% 07/12/2018
% ID fissure par RG Petrov-Galerkin et Newton, Dirichlet special case, dual force reconstruction
...
zeroboundd  = 0;      % Same for the dual problem
...
neumann3   = [3,1,fscalar/s22 ; 3,2,fscalar/s22 ; 2,1,fscalar/s22 ; 2,2,fscalar/s22 ; ...
              1,1,-fscalar/s22 ; 1,2,-fscalar/s22 ; 4,1,-fscalar/s22 ; 4,2,-fscalar/s22];
neumann4   = [3,1,-fscalar/s22 ; 3,2,fscalar/s22 ; 2,1,fscalar/s22 ; 2,2,-fscalar/s22 ; ...
              1,1,fscalar/s22 ; 1,2,-fscalar/s22 ; 4,1,-fscalar/s22 ; 4,2,fscalar/s22];
...
if theta1==0 && theta2==0
   theta1 = theta1ref; theta2 = theta2ref;
end
...
ndofd = size(f_known1(tofindN),1) + size(u_known1(tofindD),1) + 2*(ndofcrack);
...
%   end
   ...
   for pb = 0:pbmax % Construct all the problems
      ...
      Psi = sparse( 2*nboun3*Ndots, 2*nboun3 );
      ...
      %% Build the matrix of test-functions
      Vv = zeros( size(coef,1), 2*nboun3*Ndots );
      ...
      for ii=0:nmax
         ...
         for jj=0:nd1
            ...
            Vv( index, 1:2:2*nboun3*Ndots-1 )              = Wwg' .* vpaax';
            Vv( (nmax+1)^2 + index, 2:2:2*nboun3*Ndots )   = Wwg' .* vpaby';
         end
      end

      Rucij = Sv*Phi; clear Sv; clear Phi;
      Rfcij = Vv*Psi; clear Vv; clear Psi;
      Ruc = coef'*Rucij; Rfc = coef'*Rfcij;
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      Lsd = [LhsA,kB*LhsB,kB*Rfc];

      Rhs = Rhs1;
      A  = Lhs'*Lhs; b  = Lhs'*Rhs; sA = size(A,1);
      Ad = Lsd'*Lsd; bd = Lsd'*Rhs;
      ...
      if regular == 1
         ...
         Z13d = zeros( size(Duu,1), size(M3,2) ); Z23d = zeros( size(Dfu,1), size(M3,2) );
         Dtot = [ Duu ,Z12, Z13  ; Z12', kB*Dfu, Z23  ; Z13' , Z23' , D3 ];
         Ddod = [ Duu ,Z12, Z13d ; Z12', kB*Dfu, Z23d ; Z13d', Z23d', kB*M3 ];
         L = Dtot;% + Mtot * norm(Dtot,'fro')/(10*norm(Mtot,'fro'));

         L12   = [ Du(tofindD,knownD) ; zeros(size(Dfu,2),size(Duk,2)) ; zeros(2*nboun3+2,size(Duk,2)) ];
         L12d  = [ Du(tofindD,knownD) ; zeros(size(Dfu,2),size(Duk,2)) ; zeros(2*nboun3,size(Duk,2)) ];
         L2    = Du(knownD,knownD);
         ...
         L121d = L12d*u_known1(knownD);
         L122d = L12d*u_known2(knownD);
         L123d = L12d*u_known3(knownD);
         L124d = L12d*u_known4(knownD);
      else
%      L = eye(size(A));
         L = Mtot;
   ...
   %L = eye(size(A));
      ...
      MAD = Lsd'*Lsd + mur*Ddod;
      VD1 = Lsd'*Rhs1-mur*L121d; VD2 = Lsd'*Rhs2-mur*L122d;
      VD3 = Lsd'*Rhs3-mur*L123d; VD4 = Lsd'*Rhs4-mur*L124d;
      ...
      %SoluD = zeros(size(Ddod,1),4);
      %SoluD = MAD \ [VD1,VD2,VD3,VD4];

      C = zeros(nboun3, 2*nboun3);
      for i=1:nboun3
         C(i,2*i-1) = extnorm3(i,1);
         C(i,2*i)   = extnorm3(i,2);
      end
      C = [ zeros(nboun3,ndofs-2*nnodes3), C ];
      f = zeros(nboun3,4); Ctf = C'*f;
      respos = zeros(nuzawa,1); df = zeros(nuzawa,1);

      if zeroboundd == 1
         toremove = [ ndofd, ndofd-1, ndofd-2*nboun3+2, ndofd-2*nboun3+1 ];
         tokeep = setdiff(1:size(Ddod,1),toremove);
         [Ll, Uu, Pp] = lu (MAD(tokeep,tokeep));
         kuzawa1 = .999*min(eig(MAD(tokeep,tokeep)));
      else
         [Ll, Uu, Pp] = lu (MAD);  % Pp*MAD = Ll*Uu
         kuzawa1 = .999*min(eig(MAD));
      end

      for i=1:1%nuzawa % Uzawa for (f.n<0)
         if zeroboundd == 1
            SoluD = zeros(size(Ddod,1),4);
            SoluD(tokeep,:) = Uu \ ( Ll \ ( Pp * ( [VD1(tokeep),VD2(tokeep),...
                                                    VD3(tokeep),VD4(tokeep)] + Ctf(tokeep,:) ) ) );
         else
            SoluD = Uu \ ( Ll \ ( Pp * ( [VD1,VD2,VD3,VD4] + Ctf ) ) );
         end

         respos(i) = norm(C*SoluD + abs(C*SoluD),'fro');%-
         fp = f;
         if kuzawa == 0
            f = f - kuzawa1*C*SoluD;
         else
...
fcrsol1 = kB*SoluD(end-2*nboun3+1:end,1);
fcrsol2 = kB*SoluD(end-2*nboun3+1:end,2);
fcrsol3 = kB*SoluD(end-2*nboun3+1:end,3);
fcrsol4 = kB*SoluD(end-2*nboun3+1:end,4);
...
if min(size(b2nodesnoN))>0
   ...
   legend('Fx identified (1)', 'Fx reference (1)');
   end
end
% Graph for u
if min(size(b2nodesnoD))>0
   ...
   plot(toplot(1:2:end-1),'Color','red');
   plot(toplot2(1:2:end-1),'Color','blue');
   legend('Ux identified (1)', 'Ux reference (1)');
   end
end

% Graph for [[u]]
curv = sqrt( (nodes3b(:,1)-nodes3b(1,1)).^2 + (nodes3b(:,2)-nodes3b(1,2)).^2 );
mpoints = .5*( nodes3b(1:end-1,:) + nodes3b(2:end,:) );
curve = sqrt( (mpoints(:,1)-nodes3b(1,1)).^2 + (mpoints(:,2)-nodes3b(1,2)).^2 );
...
toplotf1  = fcrsol1(1:2:end-1)*n(1) + fcrsol1(2:2:end)*n(2);
toplotf2  = fcrsol2(1:2:end-1)*n(1) + fcrsol2(2:2:end)*n(2);
toplotf3  = fcrsol3(1:2:end-1)*n(1) + fcrsol3(2:2:end)*n(2);
toplotf4  = fcrsol4(1:2:end-1)*n(1) + fcrsol4(2:2:end)*n(2);
...
hold on;
set(gca, 'fontsize', 20);
plot( curve, toplotf1,'Color','red','LineWidth',3 );
legend('[[f]] identified (1)');
end
