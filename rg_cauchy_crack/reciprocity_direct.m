...
LhsA  = zeros(nftest,0); LhsB  = zeros(nftest,0);
...
for k=1:nftest
   ...
   for i=1:nboun2
      ...
      indicesA = szA+1:szA+size(umdof,1);
      ...
      for j=1:Ng
         ...
         % Remove missing info
         fer1(fmdof) = 0; uer1(umdof) = 0;
         fer2(fmdof) = 0; uer2(umdof) = 0;
         fer3(fmdof) = 0; uer3(umdof) = 0;
         fer4(fmdof) = 0; uer4(umdof) = 0;
         ...
         LhsA(k,indicesA) = LhsA(k,indicesA) + len * wg * fpa(umdof)';
...
fsolu1 = zeros(2*nboun2,1); usolu1 = zeros(2*nboun2,1);
fsolu2 = zeros(2*nboun2,1); usolu2 = zeros(2*nboun2,1);
fsolu3 = zeros(2*nboun2,1); usolu3 = zeros(2*nboun2,1);
fsolu4 = zeros(2*nboun2,1); usolu4 = zeros(2*nboun2,1);
szA = 0; szB = 0;
for i=1:nboun2
   boname = boundary(i,1);
   ...
   indicesA = szA+1:szA+size(umdof,1);
   usolu1(2*i-2+umdof) = Solu1(indicesA);
   usolu2(2*i-2+umdof) = Solu2(indicesA);
   usolu3(2*i-2+umdof) = Solu3(indicesA);
   usolu4(2*i-2+umdof) = Solu4(indicesA);
   szA = szA + size(umdof,1);
end
for i=1:nboun2
   boname = boundary(i,1);
   ...
   indicesB = szA + (szB+1:szB+size(fmdof,1));
