% 12/10/2018 : Couplage entre Cauchy et décomposition de domaines,
% Oubli des equations de Cauchy sur le bord DD et symétrisation
...
aDD     = 1; % Weight of the domain decomposition part (not symetric)
   ...
   if nbloq2{i} == 0 % nbloq < 3 is also problematic : ex if there is 1 encastred point /!\
      ...
      if sign == 1
         Gglob( [ 2*b2node3g-1, 2*b2node3g ], jj:jj+sR-1 ) = ...
                                   sign*Rloc( [ 2*b2node3s-1, 2*b2node3s ], : );
         Gglob( [2*cornoglob{i}-1, 2*cornoglob{i}], jj:jj+sR-1 ) = ...
                                                    sign*.5*Rloc( [2*corno{i}-1, 2*corno{i}], : ); % Dispatch the corner force
      else
...
for i = 1:2*nsub
   ui1 = K1{i}\f1{i};
   ui2 = K2{i}\f2{i};
   ud = zeros( 2*nnodes, 1 ); uc = zeros( 2*nnodes, 1 ); ud2 = zeros( 2*nnodes, 1 );
   ...
   ud2( [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ]  ) = ...
          - ui2( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] );
   ud2( [ 2*newbouns{el2bound(i,2)}-1, 2*newbouns{el2bound(i,2)} ]  ) = ...
          - ui2( [ 2*bounsloc{2*i}-1, 2*bounsloc{2*i} ] );

   b(4*nnodes+1:end) = b(4*nnodes+1:end) + sign*ud + aDD*sign*ud2;
   if sign == 1
      b(1:2*nnodes) = b(1:2*nnodes) + sign*uc;
   else
      b(2*nnodes+1:4*nnodes) = b(2*nnodes+1:4*nnodes) + sign*uc;
      b([2*cornoglob{i}-1, 2*cornoglob{i}] ) = b([2*cornoglob{i}-1, 2*cornoglob{i}] ) +...
                                   sign*.5*ucb([2*cornoglob{i}-1, 2*cornoglob{i}] );
   end
end
...
elseif precond == 1
   ...
   for i = 1:2*nsub
      ...
      if sign == 1 % Cauchy
         uc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = sign*Res( [2*b1to2{i}-1 ; 2*b1to2{i}] , 1 );
      else
         uc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = sign*Res( 2*nnodes + [2*b1to2{i}-1 ; 2*b1to2{i}] , 1 );
      end
...
for i = 1:2*nsub
   ...
   if sign == 1
      floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                          sign*d( [2*b1to2{i}-1 ; 2*b1to2{i}], 1 ) ;
   else
      floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                          sign*d( 2*nnodes + [2*b1to2{i}-1 ; 2*b1to2{i}], 1 ) ;
   end
   floc( [2*corno{i}-1, 2*corno{i}] ) = sign*.5*d( [2*cornoglob{i}-1, 2*cornoglob{i}], 1 ); % Dispatch the corner force
   ...
   flod( [ 2*bounsloc{2*i-1}-1; 2*bounsloc{2*i-1} ] ) = flod( [ 2*bounsloc{2*i-1}-1; 2*bounsloc{2*i-1} ] ) + ...
               sign*d( 4*nnodes+[ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], 1 );
   flod( [ 2*bounsloc{2*i}-1; 2*bounsloc{2*i} ] ) = flod( [ 2*bounsloc{2*i}-1; 2*bounsloc{2*i} ] ) + ...
               sign*d( 4*nnodes+[ 2*newbouns{el2bound(i,2)}-1, 2*newbouns{el2bound(i,2)} ], 1 );
   
   floc1 = [ floc ; zeros(nbloq1{i},1) ]; floc2 = [ floc ; zeros(nbloq2{i},1) ]; flod2 = [ flod ; zeros(nbloq2{i},1) ];
   
   ui1  = K1{i}\floc1;
   ui2t = K2{i}\[floc2,flod2]; ui2 = ui2t(:,1); uid2 = ui2t(:,2);
   ud = zeros( 2*nnodes, 1 ); uc = zeros( 2*nnodes, 1 ); ud2 = zeros( 2*nnodes, 1 );
   ...
   Ad(4*nnodes+1:end,1) = Ad(4*nnodes+1:end,1) + sign*ud + aDD*sign*ud2;
   if sign == 1
      Ad(1:2*nnodes,1) = Ad(1:2*nnodes,1) + sign*uc;
   else
      Ad(2*nnodes+1:4*nnodes,1) = Ad(2*nnodes+1:4*nnodes,1) + sign*uc;
...
for iter = 1:niter
   ...
   elseif precond == 1
      ...
      for i = 1:2*nsub
         ...
         if sign == 1
            uc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = sign*Res( [2*b1to2{i}-1 ; 2*b1to2{i}] , iter+1 );
         else
            uc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = sign*Res( 2*nnodes + [2*b1to2{i}-1 ; 2*b1to2{i}] , iter+1 );
         end
   ...
   for i = 1:2*nsub
      ...
      if sign == 1
         floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                             sign*d( [2*b1to2{i}-1 ; 2*b1to2{i}], iter+1 ) ;
      else
         floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                             sign*d( 2*nnodes + [2*b1to2{i}-1 ; 2*b1to2{i}], iter+1 ) ;
      end
      floc( [2*corno{i}-1, 2*corno{i}] ) = sign*.5*d( [2*cornoglob{i}-1, 2*cornoglob{i}], iter+1 ); % Dispatch the corner force
      ...
      Ad(4*nnodes+1:end,iter+1) = Ad(4*nnodes+1:end,iter+1) + sign*ud + aDD*sign*ud2;
      if sign == 1
         Ad(1:2*nnodes,iter+1) = Ad(1:2*nnodes,iter+1) + sign*uc;
      else
         Ad(2*nnodes+1:4*nnodes,iter+1) = Ad(2*nnodes+1:4*nnodes,iter+1) + sign*uc;
...
for i = 1:2*nsub
   ...
   if sign == 1
      floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                                sign*Lambda( [2*b1to2{i}-1 ; 2*b1to2{i}] ) ;
   else             
      floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                                sign*Lambda( 2*nnodes+[2*b1to2{i}-1 ; 2*b1to2{i}] ) ;
   end
   floc( [2*corno{i}-1, 2*corno{i}] ) = sign*.5*Lambda( [2*cornoglob{i}-1, 2*cornoglob{i}] ); % Dispatch the corner force
