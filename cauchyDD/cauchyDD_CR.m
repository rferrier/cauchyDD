% 19/09/2018 : Couplage entre Cauchy et décomposition de domaines,
% Surcharge de problèmes par pénalisation + Lagrange (Résidus Conjugués)
...
niter   = 35;
precond = 0;
alpreac = 0;%1e-2; % Reaction penalization parameter
...
for i = 1:2*nsub
   ...
   if nbloq2{i} == 0 % nbloq < 3 is also problematic : ex if there is 1 encastred point /!\
      ...
      % And the part on the Lagrange multiplier
      Gglob( 4*nnodes + [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], 3*j-2:3*j ) = ...
             sign*Rloc( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ], : );
      Gglob( 4*nnodes + [ 2*newbouns{el2bound(i,2)}-1, 2*newbouns{el2bound(i,2)} ], 3*j-2:3*j ) = ...
...
for i = 1:2*nsub
   ...
   if sign == 1
      ...
      floc( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] ) = ...
                  d( [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], 1 );
   else
      floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                          d( 2*nnodes + [2*b1to2{i}-1 ; 2*b1to2{i}], 1 ) ;

      floc( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] ) =...
                  d( 2*nnodes + [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], 1 );
   end

   flocDD( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] ) = ...
               d( 4*nnodes + [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], 1 ); % RHS For the transpose problem
   ...
   if sign == 1
      Ad(1:2*nnodes,1) = Ad(1:2*nnodes,1) + uc + sign*udt;
   else
      Ad(2*nnodes+1:4*nnodes,1) = Ad(2*nnodes+1:4*nnodes,1) + uc + sign*udt;
   end
end
CarCar = Car*Car';
Ad(:,1) = Ad(:,1) + alpreac*CarCar*Lambda; % Reaction condition
...
Az(:,1) = Az(:,1)/sqrt(den); MAd(:,1) = MAd(:,1)/sqrt(den);

%% Loop over the iterations
for iter = 1:niter
   num           = Res(:,iter)'*MAd(:,iter);
   Lambda        = Lambda + d(:,iter)*num;
   Res(:,iter+1) = Res(:,iter) - Ad(:,iter)*num;
   Rez(:,iter+1) = Rez(:,iter) - Az(:,iter)*num;
   Zed(:,iter+1) = Zed(:,iter) - MAd(:,iter)*num;
    
   if norm(Gglob) ~= 0
      %Zed(:,iter+1) = P*Zed(:,iter+1); % Its done with MAd
   ...
   for i = 1:2*nsub
      ...
      if sign == 1
         ...
         floc( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] ) =...
                     d( [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], iter+1 );
      else
         floc( [1:2:2*nno{i}-1, 2:2:2*nno{i}] ) = ...
                             d( 2*nnodes+[2*b1to2{i}-1 ; 2*b1to2{i}], iter+1 ) ;
   
         floc( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] ) =...
                     d( 2*nnodes + [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], iter+1 );
      end
   
      flocDD( [ 2*bounsloc{2*i-1}-1, 2*bounsloc{2*i-1} ] ) = ...  % RHS For the transpose problem
                  d( 4*nnodes + [ 2*newbouns{el2bound(i,1)}-1, 2*newbouns{el2bound(i,1)} ], iter+1 );
      ...
      if sign == 1
         Ad(1:2*nnodes,iter+1) = Ad(1:2*nnodes,iter+1) + uc + sign*udt;
      else
         Ad(2*nnodes+1:4*nnodes,iter+1) = Ad(2*nnodes+1:4*nnodes,iter+1) + uc + sign*udt;
      end
   end
   Ad(:,iter+1) = Ad(:,iter+1) + alpreac*CarCar*d(:,iter+1); % Reaction condition
   ...
   if norm(Gglob) ~= 0
      MAd(:,iter+1) = P*MAd(:,iter+1);
   else
      MAd(:,iter+1) = MAd(:,iter+1);
   end

   for jter = 1:iter
       betaij = ( MAd(:,iter+1)'*Ad(:,jter) );
   ...
   den = (MAd(:,iter+1)'*Ad(:,iter+1));
   d(:,iter+1) = d(:,iter+1)/sqrt(den); Ad(:,iter+1) = Ad(:,iter+1)/sqrt(den);
   Az(:,iter+1) = Az(:,iter+1)/sqrt(den); MAd(:,iter+1) = MAd(:,iter+1)/sqrt(den);
   
end
disp([ 'End of the CR ', num2str(toc) ]);
...
plotGMSH({Lambda(1:2*nnodes),'Lambda1';Lambda(2*nnodes+1:4*nnodes),'Lambda2'; ...
          Lambda(1:2*nnodes)+Lambda(2*nnodes+1:4*nnodes),'SumLambda'; ...
          Lambda(4*nnodes+1:6*nnodes),'Mu'}, elements, nodes(:,[1,2]), 'output/lambda');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
