function [u,sig,p] = Elastoplast( mat,K,f,T,nodes,elem,order,eps,nmax,varargin )
 ...
 f(1:2*nnodes,:) = f(1:2*nnodes,:) - C'*C*f(1:2*nnodes,:);
 ...
 fint   = zeros( sizf, 1 );
 lambda = zeros( 2*nnodes, 1 );
 Ktan   = K;
 
 %% Loop over time steps
 for i = 1:nt
    Deltau = zeros( sizf, 1 );
    fi     = f(:,i+1) - fint - [ lambda ; zeros(nbloq,1) ]; % Rhs for this time step
    %fi(1:2*nnodes) = fi(1:2*nnodes) - C'*C*fi(1:2*nnodes); % Suppress on bcs
...
%       if niter == 1 % The first iteration carries the Dirichlet BC
          lambda = lambda - C'*C*Ktan(1:2*nnodes,1:2*nnodes)*deltau(1:2*nnodes);
%       end
        if niter == 1 % Recompute resref
           resref = norm( f(:,i+1) - fint - [ lambda ; zeros(nbloq,1) ] );
        end
       ...
       for j = 1:nelem
          if order == 1
             ...
             Deltaepsd = [ 2/3*Deltaeps(1) - 1/3*Deltaeps(2)
                           ...
                           Deltaeps(3)/2 ]; % Deviator of Deltaeps (plane defo, e33=0)
             ...
             crit = sigeqelas - (Slim + alpha*p(j,i+1));
...
%       fint(8)
             ...
             [ lambda ; zeros(nbloq,1) ] ; % Don't re-imput the Dirichlet BC
%       fi2 = fi(2)
%       fi8 = fi(8)
       
       % Substract the internal forces to lambda ('cause it will be added later)
       lambda = lambda + C'*C*fi(1:2*nnodes);
       
       % Project fint in order to suppress loading on the BCs
       %fi(1:2*nnodes) = fi(1:2*nnodes) - C'*C*fi(1:2*nnodes);
       ...
       if niter > nmax
          stay = 0;
          warning('Newton algo terminated without the desired convergence')
       end
    end
