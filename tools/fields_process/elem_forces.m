...
% This function computes the forces per surface element from a Rhs on one boundary
 ...
 if size(boundaryR,2) == 3  % 2D
    ...
    for i=1:nboun
       ...
       ntoelem(no1) = ntoelem(no1) + len;
       ntoelem(no2) = ntoelem(no2) + len;
    end
    ...
    for i=1:nboun
       no1 = boundaryR(i,2); no2 = boundaryR(i,3);
       fe(2*indices(i)-1,:) = (fR(2*no1-1,:) + fR(2*no2-1,:));
       fe(2*indices(i),:)   = (fR(2*no1,:)   + fR(2*no2,:));
    end
 ...
 elseif size(boundaryR,2) == 4 % 3D
    ...
    for i=1:nboun
       ...
       ntoelem(no1) = ntoelem(no1) + S;
       ntoelem(no2) = ntoelem(no2) + S;
       ntoelem(no3) = ntoelem(no3) + S;
    end
    ntoelemm1 = zeros(nnodes,1);
    ntoelemm1(find(ntoelem)) = 1./ntoelem(find(ntoelem)); % Avoid the 0
    ...
    ntoelemm(2:3:3*nnodes-1) = ntoelemm1;
    ntoelemm(3:3:3*nnodes)   = ntoelemm1;
    ...
    for i=1:nboun      
       no1 = boundaryR(i,2); no2 = boundaryR(i,3); no3 = boundaryR(i,4);
       fe(3*indices(i)-2,:) = (fR(3*no1-2,:) + fR(3*no2-2,:) + fR(3*no3-2,:));
       fe(3*indices(i)-1,:) = (fR(3*no1-1,:) + fR(3*no2-1,:) + fR(3*no3-1,:));
       fe(3*indices(i),:)   = (fR(3*no1,:)   + fR(3*no2,:)   + fR(3*no3,:));
    end
