...
%% Precond
% Solve 1
f1 = dirichletRhs2( Res(:,1), 2, c2node1, boundaryp1, nnodes );
uin1 = K1\f1;
lagr1 = uin1(2*nnodes+1:end,1);
lamb1 = lagr2forces( lagr1, C1, 2, boundaryp1 );
% Solve 2
f2 = dirichletRhs2( Res(:,1), 2, c2node2, boundaryp2, nnodes );
uin2 = K2\f2;
...
for iter = 1:niter
    ...
    %% Ad1
    % Solve 1
    f1 = [d1(:,iter); zeros(nbloq1d,1)];
    uin1 = K1d\f1;
    u1i = uin1(1:2*nnodes,1);
    u1 = keepField( u1i, 2, boundaryp1 );
    % Solve 2
    f2 = [d1(:,iter); zeros(nbloq2d,1)];
    uin2 = K2d\f2;
    u2i = uin2(1:2*nnodes,1);
    u2 = keepField( u2i, 2, boundaryp2 );
    % Regularization term
    Nu = regul(d1(:,iter), nodes, boundary, 2);
    %
    Ad1(:,iter) = mu*Nu+u2-u1;
    
    %% Ad2
    % Solve 1
    f1 = [d2(:,iter); zeros(nbloq1d,1)];
    uin1 = K1d\f1;
    u1i = uin1(1:2*nnodes,1);
    u1 = keepField( u1i, 2, boundaryp1 );
    % Solve 2
    f2 = [d2(:,iter); zeros(nbloq2d,1)];
    uin2 = K2d\f2;
    u2i = uin2(1:2*nnodes,1);
    u2 = keepField( u2i, 2, boundaryp2 );
    % Regularization term
    Nu = regul(d2(:,iter), nodes, boundary, 2);
    %
    Ad2(:,iter) = mu*Nu+u2-u1;
    ...
    %% Precond
    % Solve 1
    f1 = dirichletRhs2( Res(:,iter+1), 2, c2node1, boundaryp1, nnodes );
    uin1 = K1\f1;
    lagr1 = uin1(2*nnodes+1:end,1);
    lamb1 = lagr2forces( lagr1, C1, 2, boundaryp1 );
    % Solve 2
    f2 = dirichletRhs2( Res(:,iter+1), 2, c2node2, boundaryp2, nnodes );
    uin2 = K2\f2;
    lagr2 = uin2(2*nnodes+1:end,1);
    lamb2 = lagr2forces( lagr2, C2, 2, boundaryp2 );
    %
