...
H  = zeros(2*niter);
eta = [0,0;0,0]; % useless, but eta needs initialization #lazy
num = [0,0;0,0];
den = [0,0;0,0];
%%
for iter = 1:niter
...
%                                    Ad(:,[2*iter-1,2*iter])*alpha;
    ...
    % Ritz variables (Saad++)
%    alpha( [2*iter-1,2*iter], [2*iter-1,2*iter] ) = sqD\num;
...
%         - sqD \ ( Zed(indexa,[2*iter+1,2*iter+2])'*Ad(indexa,[2*iter-1,2*iter]) );
    beta( [2*iter-1,2*iter], [2*iter-1,2*iter] )  = ...
    ...
    if iter > 1
       ...
       etaeta( [2*iter-1,2*iter], [2*iter-1,2*iter] ) = ...
               (denprec^(1/2)*numprec)^(-1/2) * denprec^(1/2)*numprec * ...
                    (sqD*numprec)^(1/2);  % TODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUGTODEBUG
    end
...
%      ((Res(indexa,[2*iter-1,2*iter])'*Zed(indexa,[2*iter-1,2*iter]))^(-1/2)) ;
    ...
    etap   = eta;
%    delta  = inv( alpha( [2*iter-1,2*iter], [2*iter-1,2*iter] ) );
    delta  = unsuralpha( [2*iter-1,2*iter], [2*iter-1,2*iter] ) ;
    if iter > 1
%       delta  = delta + alpha([2*iter-3,2*iter-2],[2*iter-3,2*iter-2]) \ ...
%                   beta([2*iter-3,2*iter-2],[2*iter-3,2*iter-2]);
       delta = delta + betasuralpha( [2*iter-3,2*iter-2], [2*iter-3,2*iter-2] );
%       delta  = delta + beta([2*iter-3,2*iter-2],[2*iter-3,2*iter-2]) * ...
%                        inv ( alpha([2*iter-3,2*iter-2],[2*iter-3,2*iter-2]) ) ;
    end
%    eta    = alpha( [2*iter-1,2*iter], [2*iter-1,2*iter] ) \ ...
%                   beta( [2*iter-1,2*iter], [2*iter-1,2*iter] )^(1/2);
    eta = etaeta( [2*iter-1,2*iter], [2*iter-1,2*iter] ); % what a stupid variable name
%    eta    = beta( [2*iter-1,2*iter], [2*iter-1,2*iter] )^(1/2) * ...
%                  inv( alpha( [2*iter-1,2*iter], [2*iter-1,2*iter] ) ) ;
    
    if iter > 1
       H( [2*iter-1,2*iter] , [2*iter-3,2*iter-2,2*iter-1,2*iter] ) = ... 
                                                              [etap, delta];
       H( [2*iter-3,2*iter-2] , [2*iter-1,2*iter] ) = etap;
    else
